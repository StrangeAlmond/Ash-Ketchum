...
module.exports = {
    ...
    async execute(message, args, bot) {
        ...
        const embed = new Discord.MessageEmbed()
            .setAuthor(`${message.guild.name}'s ${board.charAt(0).toUpperCase() + board.slice(1)} Leaderboard`,
                message.guild.iconURL)
            .setTimestamp();

        for (const user of users) {
            embed.addField(`${message.guild.members.cache.get(user.user).displayName}`,
